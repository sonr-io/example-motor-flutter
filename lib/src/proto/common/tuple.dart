...
/// ### StringComparable
/// Simple `String` Utility for Comparing Values
extension StringComparable on String {
  /// Returns this String as capitalized first word and lowercase remaining
  String capitalize() => this.substring(0, 1).toUpperCase() + this.substring(1).toLowerCase();

  /// Returns substring of this string while removing all characters
  /// AFTER given value
  String cutAfter(String val) => this.contains(val) ? this.substring(0, indexOf(val)) : this;

  /// Returns substring of this string while removing all characters
  /// BEFORE given value
  String cutBefore(String val) => this.contains(val) ? this.substring(indexOf(val) + 1) : this;

  /// Checks if this String is equal to given value.
  /// When [`loose`] is enabled value is compared to as same
  /// case characters.
  bool equals(String e, {bool loose = false}) => loose ? e.toLowerCase() == this.toLowerCase() : e == this;

  /// Checks if this String is equal to ANY of the given values
  /// When [`loose`] is enabled value is compared to as same
  /// case characters.
  bool equalsAny(List<String> val, {bool loose = false}) => val.any(
        (e) => loose ? e.toLowerCase() == this.toLowerCase() : e == this,
      );

  /// Checks if this String is equal to EVERY one of the given values
  /// When [`loose`] is enabled value is compared to as same
  /// case characters.
  bool equalsEvery(List<String> val, {bool loose = false}) => val.every(
        (e) => loose ? e.toLowerCase() == this.toLowerCase() : e == this,
      );
}
